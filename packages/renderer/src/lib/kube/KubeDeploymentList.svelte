...
import { filtered, searchPattern } from '../../stores/kubernetesDeployments';
...
// Store
let deployments: DeploymentUIInfo[] = [];
...
<NavPage bind:searchTerm="{searchTerm}" title="deployments">
  <div class="min-w-full flex" slot="table">
    <table class="mx-5 w-full" class:hidden="{deployments.length === 0}">
      <!-- title -->
      <thead>
        <tr class="h-7 uppercase text-xs text-gray-600">
          ...
          <th class="whitespace-nowrap px-6">Pods</th>
            ...
            <td class="whitespace-nowrap w-10 hover:cursor-pointer">
              <div class="flex items-center">
                <div class="flex flex-row items-center">
                  <div class="text-sm text-gray-300">{deployment.name}</div>
                </div>
              </div>
            </td>
            <td class="px-6 py-2 whitespace-nowrap w-10">
              <div class="flex items-center">
                <div class="text-sm text-gray-600">{deployment.namespace}</div>
              </div>
            </td>
            <td class="px-6 py-2 whitespace-nowrap w-10">
              <div class="flex items-center">
                <div class="text-sm text-gray-600">{deployment.ready} / {deployment.replicas}</div>
              </div>
            </td>
            <td class="pl-6 text-right whitespace-nowrap rounded-tr-lg rounded-br-lg">
              <div class="flex w-full">
                ...
                <div class="text-right w-full">
                  <ListItemButtonIcon title="Delete Deployment" icon="{faTrash}" />
                </div>
              </div>
            </td>
          </tr>

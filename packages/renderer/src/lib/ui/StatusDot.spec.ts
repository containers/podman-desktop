...
const renderStatusDot = (containerStatus: string) => {
  return render(StatusDot, { container: { Names: 'foobar', Id: 'foobar', Status: containerStatus } });
};
...
test('Expect the dot to have the correct color for degraded status', () => {
  ...
  const dot = screen.getByTestId('status-dot');
  expect(dot).toHaveClass('bg-amber-700');
});

test('Expect the dot to default to gray for unknown status', () => {

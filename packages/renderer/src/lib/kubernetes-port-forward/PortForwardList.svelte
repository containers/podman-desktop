...
const columns = [
  new TableColumn<PortForwardRow, string>('Name', {
    align: 'left',
  ...
  new TableColumn<PortForwardRow, { name: string; namespace: string }>('Kube Object', {
    align: 'left',
...
const row = new TableRow<PortForwardRow>({ selectable: _deployment => true });

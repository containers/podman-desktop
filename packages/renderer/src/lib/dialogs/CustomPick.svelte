              ...
              {#each Array(colsPerRow) as _, j}
                ...
                <div
                  ...
                  <div
                    ...
                    class:border-b-2={usePopperForDetails || itemSectionHiddenStatus.get((i/colsPerRow)*colsPerRow+j)}
                    class:rounded-b-md={usePopperForDetails || itemSectionHiddenStatus.get((i/colsPerRow)*colsPerRow+j)}
                    ...
                    {#if usePopperForDetails || (innerItem.sections && innerItem.sections.length > 0 && itemSectionHiddenStatus.get((i/colsPerRow)*colsPerRow+j))}
                      <div
                        ...
                        class:invisible={usePopperForDetails && !itemSectionHiddenStatus.get((i/colsPerRow)*colsPerRow+j)}>
                        <button
                  ...
                  {#if innerItem.sections}                  
                    <div class:relative={usePopperForDetails}
                    on:mousedown="{e => {
                      if(usePopperForDetails || itemSectionHiddenStatus.get((i/colsPerRow)*colsPerRow+j)) {
                        e.stopPropagation();
                      }                    
                    }}">
                    <div class="flex flex-col {usePopperForDetails ? 'group-[.hide-section]/footer:hidden' : 'group-[.hide-section]/footer:invisible'} rounded-md"
                      ...
                      <div class="flex justify-center py-2 text-xs font-bold group-[.is-selected]:bg-purple-600 group-hover:bg-purple-600 border-x-2 border-transparent group-hover:border-purple-500
                      bg-charcoal-300 rounded-t-md"
                      class:rounded-t-md={usePopperForDetails && i === 0}
                      >
                        {section.title}
                      </div>
                      {#if section.content}
                        <div
                          ...
                          class:rounded-b-md={usePopperForDetails && !section.markDownContent && i === innerItem.sections.length - 1}
                        >
                          {section.content}
                        </div>
                      {/if}

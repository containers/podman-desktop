...
export class CreateKindClusterPage extends BasePage {
  ...
  readonly clusterIngressControllerCheckbox: Locator;
  ...
  constructor(page: Page) {
    ...
    this.clusterIngressControllerCheckbox = this.clusterPropertiesInformation.getByRole('checkbox', {
    ...
    this.checkboxHolder = this.clusterIngressControllerCheckbox.locator('..');
  ...
  public async manageCheckboxState(operation: CheckboxOperations): Promise<void> {
    await playExpect(this.clusterIngressControllerCheckbox).toBeVisible();
    switch (operation) {
      case CheckboxOperations.Check:
        await this.checkboxHolder.check();
        await playExpect(this.clusterIngressControllerCheckbox).toBeChecked();
        break;
      case CheckboxOperations.Uncheck:
        await this.checkboxHolder.uncheck();
        await playExpect(this.clusterIngressControllerCheckbox).not.toBeChecked();
        break;
    }
  }
}

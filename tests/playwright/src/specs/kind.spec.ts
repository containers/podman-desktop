...
const KUBERNETES_KONTEXT: string = `kind-${KIND_CLUSTER}`;
...
describe('Kind End-to-End Tests', async () => {
  ...
  describe('Kind cluster creation', async () => {
    test('Create a Kind cluster', async () => {
      ...
      await createKindClusterPage.manageCheckboxState(CheckboxOperations.Uncheck);
    ...
    test('Check for resources added with the Kind cluster', async () => {
      ...
      playExpect(await containerDetailsPage.getState()).toEqual(ContainerState.Running.toLowerCase());
    ...
    test('Validate the right Kubernetes context is chosen', async () => {
      await statusBar.validateKubernetesContext(KUBERNETES_KONTEXT);
    });

...
test.describe('Kubernetes Edit YAML Feature E2E Test', () => {
  test.skip(
  ...
  test('Change the Kubernetes deployment YAML file', async ({ navigationBar }) => {
    test.setTimeout(80_000);
    const podsPage = await navigationBar.openPods();
    await playExpect.poll(async () => await podsPage.countPodReplicas(DEPLOYMENT_NAME)).toEqual(3);
    ...
    await playExpect.poll(async () => await podsPage.countPodReplicas(DEPLOYMENT_NAME)).toBe(5);

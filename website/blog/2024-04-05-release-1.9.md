...
- **Podman 5 !** [Podman 5.0.1](https://github.com/containers/podman/releases/tag/v5.0.1) as an experimental feature for existing users or set as default for new users.
...
🦭 Podman version 5 is now accessible as an experimental feature for existing Podman users and is set as the default for new users.

Why experimental for existing users ? The main reason is that a Podman machine version 4 is not compliant at all with a Podman machine version 5.
...
For more details on the save/load feature, refer to the release notes.

![v5 update](img/podman-desktop-release-1.9/podman-update-to-v5.png)

**_NOTE:_** On macOS you will notice a lot of performance enhancements as Apple Hypervisor is used instead of QEMU for the Podman machine. Expect better performance on the file sharing.

Here is a blog post introducing Podman v5 https://blog.podman.io/2024/03/podman-5-0-has-been-released/

#### Enabling experimental flag

If you're running a Podman version 4, to see the update to version 5 you need to enable a flag `v5` in Podman extension preferences.
...
Because Podman v5 is not able to read Podman v4 machines, before updating, backup images that you need to keep. Maybe you don't need to save images if they are all
available on remote registries and transient.
...
**_NOTE:_** Exporting the filesystem of containers only export the content of the filesystem. Importing will result in a container without any commands, so this might not be what you expect.
...
Also we published a test framework to test extension in separate repositories

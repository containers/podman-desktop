...
- **Improved Feedback form**: We redesigned the feedback form to ease reporting issues to GitHub 🪲
- **New Experimental Task Manager**: A new experimental Task Manager has arrived 🔔 ! 
- **More Kubernetes events**: We added the support of Events for many resources such as the Nodes, Services, Pods and more 📜!
- **Interact with the Podman Machine**: You can now ssh in the podman machine from Podman Desktop.

## Release details 🔍

### Improved Feedback form 📝

With the 1.15 release, we introduced a new Feedback form, with the growing number of user, we wanted to provide a way to 
easily redirect to the GitHub issues of our main repository [podman-desktop/podman-desktop/issues](https://github.com/podman-desktop/podman-desktop/issues) 

We have our internal feedback system, allowing us to collect anonymous feedback, we all wanted to keep that, because it
allows any user to send us their feeling, or potential issue without needed a GitHub account. But now we allow also to preview the problem in GitHub.
...
We now distinguish 3 categories `Direct your words the developpers`, `Feature request` and `Bug`. The first one
will remain anonymous, and only the developers will have access. The feature request and bug categories will redirect you to GitHub.
...
Long time ago, in the [0.13 Release Notes](/blog/podman-desktop-release-0.13#new-task-manager-1724) we introduced a Task Manager,
with the growing number of long-running tasks, we revamped the task manager, and the result is now available as an experimental feature!

#### Enable the experimental task manager

In the `Preferences > Tasks` page you have a new `Manager`  option. Enable it to test the new task manager !
...
Some of the changes include more usage of the width, improved consistency with the application, more capabilities on individual tasks etc.
...
We started early this year to introduce more Kubernetes capabilities to Podman Desktop, not as a standalone feature, but as an integrate part of the application,
this ongoing effort continue and in this release, we addressed a lot of technical debt and bugs, but we also added the support of the [Kubernetes Events](https://kubernetes.io/docs/reference/kubernetes-api/cluster-resources/event-v1/).
...
### Interact with the Podman Machine

One pain point we often hear from user is to be able to easily get a ssh connection to a podman machine, to either debug, configure etc. 
From the machine details you can now connect directly to the machine
...
* docs(website): edited the managing objects section by @shipsing in https://github.com/podman-desktop/podman-desktop/pull/9845

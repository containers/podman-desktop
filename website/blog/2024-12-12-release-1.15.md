...
1. **Direct Message to Developers**: Keeps feedback anonymous, visible only to our team.
2. **Feature Request**: Redirects users to GitHub to suggest new features.
3. **Bug Report**: Allows users to report bugs directly on GitHub.
...
To try it out, go to `Preferences > Tasks` and enable the `Manager` option.

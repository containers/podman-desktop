...
- **Kubernetes improvements with new dashboard**: A new landing screen for Kubernetes has been added with UI changes that will make life with working with Kubernetes clusters easier.
- **Port forwarding**: This new feature allows users to configure port forwarding in their Kubernetes environment.
<!--truncate-->
...
### Kubernetes improvements with new dashboard

We have updated the Kubernetes dashboard page to give to provide a quick overview of users' Kubernetes clusters, alongside with multiple changes to Kubernetes backend.  

![kubernetes dashboard](img/podman-desktop-release-1.14/kubernetes_dashboard.png)

### Port forwarding

Podman Desktop now supports port forwarding for pods in Kubernetes environments. Port forwarding can be done from pod detail page and then visible in the Port forwarding page. 

...
describe('Podman5DownloadMachineOS', () => {
  ...
  test('download all the files and perform checks on Windows', async () => {
    ...
    nock('https://quay.io').get('/v2/podman/machine-os-wsl/manifests/1.0-fake').reply(200, rootManifest);

    // fake digest for amd64
    nock('https://quay.io')
      .get('/v2/podman/machine-os-wsl/manifests/sha256:123amd64')
      .reply(200, {
        schemaVersion: 2,
        mediaType: 'application/vnd.oci.image.manifest.v1+json',
        config: {
    ...
    // fake digest for arm64
    nock('https://quay.io')
      .get('/v2/podman/machine-os-wsl/manifests/sha256:456arm64')
      .reply(200, {
        schemaVersion: 2,
        mediaType: 'application/vnd.oci.image.manifest.v1+json',
        config: {
    ...
    // now do the digests for blobs
    nock('https://quay.io')
      .get('/v2/podman/machine-os-wsl/blobs/sha256:zstfakeamd64digest')
      .reply(200, 'fake-amd64-content');

    const zstdArchiveFakeContent = 'blablabla-ARM64\n';
    nock('https://quay.io')
      .get('/v2/podman/machine-os-wsl/blobs/sha256:zstfakearm64digest')
      .reply(200, zstdArchiveFakeContent, {
        'content-type': 'application/octet-stream',
        'content-length': `${zstdArchiveFakeContent.length}`,
        'content-disposition': 'attachment; filename=binary.zip',
